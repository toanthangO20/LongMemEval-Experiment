...
if [[ $model_alias == "gpt-4o" || $model_alias == "gpt-4o-mini" ]]; then
    ...
    if [[ -n "$OPENAI_BASE_URL" ]]; then
        openai_base_url_flag="--openai_base_url ${OPENAI_BASE_URL}"
    fi
    openai_key="${OPENAI_API_KEY:-YOUR_API_KEY}"
    openai_org_flag=""
else
    # will call an openai server emulator served by e.g., vllm locally
    openai_base_url_flag="--openai_base_url ${OPENAI_BASE_URL:-http://localhost:8001/v1}"
    openai_key="${OPENAI_API_KEY:-EMPTY}"
    openai_org_flag=""
fi
...
python run_generation.py \
       ...
       ${openai_org_flag} ${openai_base_url_flag}  \
